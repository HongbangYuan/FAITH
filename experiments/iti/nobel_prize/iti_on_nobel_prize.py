...
class ItiInferencer:
    ...
    def get_head_wise_features(self,samples,question_key,answer_key,label_key,head_feature_cache_file,rewrite=False):
        ...
        if rewrite or not os.path.isfile(head_feature_cache_file):
            print("Constructing cache files...")
    ...
    def run_experiment(self,samples,input_keys,output_keys,intervene,layers_to_intervene,batch_size):
        model,tokenizer = self.model,self.tokenizer
        total = len(input_keys)
        for i,(input_key, output_key) in enumerate(zip(input_keys, output_keys)):
            ...
            with torch.no_grad():
                ...
                for batch in tqdm(data_loader):
                    prompts = list(map(lambda x: format_question(x, instruction=input_keys2instructions[input_key]),
                                       batch[input_key]))
...
if __name__ == '__main__':
    ...
    result_dir = f'{base_dir}/results/iti/NobelPrize/{model_size}'
    ...
    results = inferencer.get_head_wise_features(
        samples,
    ...
    print("Search top heads...")
    intervene,layers = inferencer.choose_top_head(
        results,
        test_size,
        random_seed,
        max_iter,
    ...
    print("Begin running experiments...")
