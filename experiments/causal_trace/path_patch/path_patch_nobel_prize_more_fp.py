...
def calculate_path_patch(
    ...
    differences = []
    for selected_layer in tqdm(layers[:num_hidden_layers//2]):
        row = []
        # for selected_head in tqdm(range(num_heads), leave=False):
        for selected_head in range(num_heads):
            ...
            def patch_rep(x, layer):
                ...
                h[2, end_of_false_premise_pos, :] = h[0, end_of_false_premise_pos, :]
                if layer in selected_layer:
                    # except for the sender head whose activation is on counter factual data
                    h[2, end_of_false_premise_pos, dim_start:dim_end] = h[1, end_of_knocked_false_premise_pos, dim_start:dim_end]
                return x
...
if __name__ == '__main__':
    ...
    parser.add_argument('--question_number',type=int,default=2,choices=[2,3,4])
    ...
    result_dir = f'{base_dir}/results/causal_trace/path_patch/NobelPrize/more_fps/head_contributions_{model_size}/question_{question_number}'
    ...
    dataset_file = f"{base_dir}/results/causal_trace/tok_pred/NobelPrize/llama2-{model_size}-chat_on_nobel_prize_more_fps_when_fp_question_token_answer.json"
