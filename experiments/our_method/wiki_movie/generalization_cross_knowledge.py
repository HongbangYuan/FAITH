...
if __name__ == '__main__':
    ...
    result_dir = f'{base_dir}/results/causal_trace/knockout/Movies/more_fps/across_knowledge'
    ...
    for q in range(1,4+1):
        print(f"--------------q={q}------------------")
        path_patch_result_dir = f'{base_dir}/results/causal_trace/path_patch/NobelPrize/more_fps/head_contributions_{model_size}/question_{q}'
        if q == 1:
            path_patch_result_dir = f'{base_dir}/results/causal_trace/path_patch/NobelPrize/head_contributions_{model_size}'
    ...
    for q in range(1,4+1):
        ...
        tag = f'Q_{question_number}_with_head_from_prize_Q{q}_threshold_{str(threshold)}_topk_{str(top_k)}'
